...
class Template_pesan extends CI_Controller
{
    ...
    public function edit($ID_TEMPLATE)
    {
        ...
        $data['acara'] = $this->m_acara->tampil_data()->result();
        //membuat function yg digunakan dimodal
        $data['template_pesan'] = $this->m_template->edit_data($ID_TEMPLATE, 'template_pesan')->result(); //m_acara = nama modalnya, masukkan nma function edit_data

        $this->load->view('template/header'); //u/ngeload view dari folder template
        $this->load->view('template/sidebar');
        $this->load->view('edit_template_pesan', $data);
        $this->load->view('template/footer');
    }
    public function update()
    {
        ...
        $where1 = array('ID_TEMPLATE' => $ID_TEMPLATE);
        $this->m_template->update_data($where1, $data1, 'template_pesan');
